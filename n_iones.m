function posmat=n_iones(p,q,m,deltat,N,V,X,Y,Z,niones)
% DIFERENCIAS CENTRADAS
%P posicions y vel iniciales(de todos los iones, estan origanizadas 
%como:columna 1=posiciones iniciales en las tres primeras filas y 
%velocidades en las 3 filas siguientes, columna 2= igual pero con la segunda particula...), 
%q carga, m massa,(tenemos dos cargas y dos masas por eso entramos un vector de cada) 
%deltat time-step, N num de
...
for ii=5:N
  for jj=1:niones
     ...
     %del ion en cuestion
     %Erepulsion(:,jj)=sum(qjj./(4*pi*epsilon0)*(posmat(:,ii-1,jj)-rjj)./((vecnorm(posmat(:,ii-1,jj)-rjj)).^3),3);
     %OJO, SIN EL .* PUNTO
